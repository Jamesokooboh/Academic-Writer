...
You will receive a JSON object with "writing_mode", "rewrite_strength", "context" (the \
preceding paragraph's tail, for local coherence only — do not rewrite it), and "sentences" \
(a list of {"id": <id>, "text": <text>} to rewrite).
...
def _build_prompt(batch: list[RewriteRequest], context: str, writing_mode: str, rewrite_strength: str) -> str:
    payload = {
...
async def rewrite_batch(
    ...
    prompt = _build_prompt(batch, context, writing_mode, rewrite_strength)
